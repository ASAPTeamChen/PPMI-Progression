...
# Fit a linear mixed-effects model
caudate_saa2 <- lmer(
  sbr_caudate ~ visit_year * `SAA Result` * `Group ID` + baseline_sbr_caudate + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + upd23a_medication_for_pd + `Group ID` + (1 | participant_id),
  data = data_datscan_saa
)
...
plot_caudate_saa2 <- sjPlot::plot_model(caudate_saa2, type = "pred", ci.lvl = NA, terms = c("visit_year", "SAA Result", "Group ID")) + 
  ...
  scale_color_brewer(palette = "Set1") +
  scale_y_continuous(limits = c(0.8, 2)) +  # Set y-axis range
  scale_x_continuous(limits = c(0, 5))  # Set x-axis range
...
# Fit a linear mixed-effects model
putamen_saa2 <- lmer(
  sbr_putamen ~ visit_year * `SAA Result` * `Group ID` + baseline_sbr_putamen + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + upd23a_medication_for_pd + `Group ID` + (1 | participant_id),
  data = data_datscan_saa
)
...
plot_putamen_saa2 <- sjPlot::plot_model(putamen_saa2, type = "pred", ci.lvl = NA, terms = c("visit_year", "SAA Result", "Group ID")) + 
  ...
  scale_color_brewer(palette = "Set1") +
  scale_y_continuous(limits = c(0.2, 1)) +  # Set y-axis range
  scale_x_continuous(limits = c(0, 5))  # Set x-axis range
