...
```{r}
file_path <- "/Users/jackson/Desktop/LRRK2 MS Materials/LRRK2 PPMI (2023 V4) Analysis Data (Latest) (GBA Removed).xlsx"
...
# Merge the selected columns into data_moca
...
histogram_visit_year <- ggplot(data_moca, aes(x = visit_year)) +
...
Use data_moca as the main analysis dataframe for future analysis.

```{r, eval=FALSE}
save(data_moca, file = "data_moca.RData")
```
...
moca_model1 <- lmer(
  MoCA_score ~ visit_year * `Group ID` + baseline_score + (1 | participant_id),
  data = data_moca
)
...
tab_model(moca_model1, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```

```{r}
# Filter out participants with only one measurement
filtered_data <- data_moca %>%
  group_by(participant_id) %>%
  filter(n() > 1) %>%
  ungroup()

# Fit the linear mixed-effects model on filtered data
moca_model_filtered <- lmer(
  MoCA_score ~ visit_year * `Group ID` + baseline_score + (1 | participant_id),
  data = filtered_data
)

tab_model(moca_model_filtered, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```
...
head(data_moca_summary)

# Create the plot
plot_moca_model1 <- ggplot(data_moca_summary, aes(x = visit_year_median, y = mean_score, color = `Group ID`, group = `Group ID`)) +
...
# Extract the legend
legend <- ggpubr::get_legend(plot_moca_model1)
...
Multivariable adjusted progression of MoCA_score in Parkinson’s Disease patients carrying different LRRK2 mutations.

```{r}
# Fit a linear mixed-effects model
moca_model2 <- lmer(
  MoCA_score ~ visit_year * `Group ID` + baseline_score + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + (1 | participant_id),
  data = data_moca
)

# Create a publishable table using sjPlot
tab_model(moca_model2, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```
...
plot_moca_model2 <- sjPlot::plot_model(moca_model2, type = "pred", ci.lvl = NA, terms = c("visit_year", "Group ID"), colors = "Set1") +
  ...
  scale_y_continuous(limits = c(15, 50)) + # Set y-axis range 
...
```{r}
data_moca_saa <- data_moca %>%
  filter(`SAA Result` %in% c("Negative", "Positive"))

moca_saa1 <- lmer(
  MoCA_score ~ visit_year * `SAA Result` + baseline_score + (1 | participant_id),
  data = data_moca_saa
)

tab_model(moca_saa1, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```

```{r}
# Categorize visit_year into bins of every 12 months
data_moca_saa <- data_moca_saa %>%
  mutate(visit_bin = cut(visit_year, breaks = seq(0, max(visit_year, na.rm = TRUE), by = 1), include.lowest = TRUE, right = FALSE))

# Aggregate the data by visit_bin and Group ID
data_moca_saa_summary <- data_moca_saa %>%
  group_by(visit_bin, `SAA Result`) %>%
  summarise(
    mean_score = mean(MoCA_score, na.rm = TRUE),
    sd_score = sd(MoCA_score, na.rm = TRUE),
    n = n(),
...
# Display the summary data
head(data_moca_saa_summary)

# Create the plot
plot_moca_saa1 <- ggplot(data_moca_saa_summary, aes(x = visit_year_median, y = mean_score, color = `SAA Result`, group = `SAA Result`)) +
  geom_line(linewidth = 0.25) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = mean_score - se_score, ymax = mean_score + se_score), width = 0.5) +
  theme_classic() + 
...
# Extract the legend
legend_saa <- ggpubr::get_legend(plot_moca_saa1)
```
...
```{r}
moca_saa2 <- lmer(
  MoCA_score ~ visit_year * `SAA Result` + baseline_score + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + (1 | participant_id),
  data = data_moca_saa
)

tab_model(moca_saa2, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```
## Model 3: Additionally adjusted for LRRK2

```{r}
# Fit a linear mixed-effects model
moca_saa3 <- lmer(
  MoCA_score ~ visit_year * `SAA Result` + baseline_score + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + `Group ID` + (1 | participant_id),
  data = data_moca_saa
)

tab_model(moca_saa3, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```

```{r}
# Visualize the fixed effects of the model
plot_moca_saa3 <- sjPlot::plot_model(moca_saa3, ci.lvl = NA, type = "pred", terms = c("visit_year", "SAA Result")) + 
  geom_line(linewidth = 0.25) +
  theme_classic() + 
  labs(
    x = "Visit month",
...
## SAA * LRRK2 interaction

Significant

```{r}
moca_saa4 <- lmer(
  MoCA_score ~ visit_year * `SAA Result` * `Group ID` + baseline_score + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + (1 | participant_id),
  data = data_moca_saa
)

tab_model(moca_saa4, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")

# Perform an ANOVA to compare the models
anova(moca_saa3, moca_saa4)
```

### Figure 2

```{r}
# Visualize the fixed effects of the model for the interaction terms
plot_moca_saa4 <- sjPlot::plot_model(moca_saa4, type = "pred", ci.lvl = NA, terms = c("visit_year", "SAA Result", "Group ID"), colors = "Set1") + 
  geom_line(linewidth = 0.25) +
  theme_classic() + 
  labs(
    x = "Visit month",
    y = "MoCA_score",
    color = "Interaction: α-synuclein SAA * LRRK2 mutation"
  ) + 
  theme(
    legend.position = "bottom",
    plot.title = element_blank()
  ) + 
  scale_y_continuous(limits = c(10, 50)) +  # Set y-axis range
  scale_x_continuous(limits = c(0, 15))  # Set x-axis range

# Display the plot
print(plot_moca_saa4)
