...
plot_updrs_model2 <- sjPlot::plot_model(updrs_model2, type = "pred", ci.lvl = NA, terms = c("visit_year", "Group ID"), colors = "Set2") +
...
```{r}
data_updrs_saa <- data_updrs %>%
  filter(`SAA Result` %in% c("Negative", "Positive"))

updrs_saa1 <- lmer(
  mds_updrs_part_iii_summary_score ~ visit_year * `SAA Result` + baseline_score + (1 | participant_id),
  data = data_updrs_saa
)

tab_model(updrs_saa1, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```

```{r}
# Categorize visit_year into bins of every 12 months
data_updrs_saa <- data_updrs_saa %>%
  mutate(visit_bin = cut(visit_year, breaks = seq(0, max(visit_year, na.rm = TRUE), by = 1), include.lowest = TRUE, right = FALSE))

# Aggregate the data by visit_bin and Group ID
data_updrs_saa_summary <- data_updrs_saa %>%
...
# Display the summary data
head(data_updrs_saa_summary)

# Create the plot
plot_updrs_saa1 <- ggplot(data_updrs_saa_summary, aes(x = visit_year_median, y = mean_score, color = `SAA Result`, group = `SAA Result`)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = mean_score - se_score, ymax = mean_score + se_score), width = 0.5) +
  theme_classic() + 
...
# Extract the legend
legend_saa <- ggpubr::get_legend(plot_updrs_saa1)
```
...
```{r}
updrs_saa2 <- lmer(
  mds_updrs_part_iii_summary_score ~ visit_year * `SAA Result` + baseline_score + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + upd23a_medication_for_pd + (1 | participant_id),
  data = data_updrs_saa
)

tab_model(updrs_saa2, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```
## Model 3: Additionally adjusted for LRRK2 and GBA Mutation

```{r}
# Fit a linear mixed-effects model
updrs_saa3 <- lmer(
  mds_updrs_part_iii_summary_score ~ visit_year * `SAA Result` + baseline_score + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + upd23a_medication_for_pd + `Group ID` + (1 | participant_id),
  data = data_updrs_saa
)

tab_model(updrs_saa3, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")
```

```{r}
# Visualize the fixed effects of the model
plot_updrs_saa3 <- sjPlot::plot_model(updrs_saa3, ci.lvl = NA, type = "pred", terms = c("visit_year", "SAA Result")) + 
  geom_line(linewidth = 0.25) +
  theme_classic() + 
  labs(
    x = "Visit month",
...
```{r}
updrs_saa4 <- lmer(
  mds_updrs_part_iii_summary_score ~ visit_year * `SAA Result` * `Group ID` + baseline_score + `Age at baseline` + Sex + Race + Ethnicity + `Years of Education` + upd23a_medication_for_pd + (1 | participant_id),
  data = data_updrs_saa
)

tab_model(updrs_saa4, show.re.var = FALSE, show.icc = FALSE, show.r2 = FALSE, 
          title = "Model Coefficients and Slopes for Time Effect by Groups")

# Perform an ANOVA to compare the models
anova(updrs_saa3, updrs_saa4)
```
...
plot_updrs_saa4 <- sjPlot::plot_model(updrs_saa4, type = "pred", ci.lvl = NA, terms = c("visit_year", "SAA Result", "Group ID"), colors = "Set1") + 
  geom_line(linewidth = 0.25) +
  theme_classic() + 
  labs(
    x = "Visit month",
  ...
  scale_y_continuous(limits = c(10, 60, 10)) +  # Set y-axis range
  scale_x_continuous(limits = c(0, 15))  # Set x-axis range

# Display the plot
print(plot_updrs_saa4)
